...
_Avoid_: "quest bubble" (it reports need, not objectives); reading it as a HUD element (it is diegetic and world-space); assuming it implies dead-door **deregistration** — diagnosis and cleanup are deliberately separate, and only diagnosis is intended (ADR-0011); adding a retry backoff (considered and declined).
